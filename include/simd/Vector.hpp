...
inline Vector<int32_t, 4> operator<<(const Vector<int32_t, 4>& a, const int& b) {
	// The Arm NEON intrinsic requires 'b' to be a compile time constant, x86 doesn't
	switch(b)
	{
#define CASE(_b) case ((_b)): return Vector<int32_t, 4>(simde_mm_slli_epi32(a.v, (_b)))
		...
		default:
			return Vector<int32_t, 4>(simde_mm_setzero_si128());
#undef CASE
	}
}
...
inline Vector<int32_t, 4> operator>>(const Vector<int32_t, 4>& a, const int& b) {
	...
	switch(b)
	{
#define CASE(_b) case ((_b)): return Vector<int32_t, 4>(simde_mm_srli_epi32(a.v, (_b)))
		CASE(0); CASE(1); CASE(2); CASE(3);
		CASE(4); CASE(5); CASE(6); CASE(7);
		CASE(8); CASE(9); CASE(10); CASE(11);
		CASE(12); CASE(13); CASE(14); CASE(15);
